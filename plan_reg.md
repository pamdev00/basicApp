...
* [x] `ResendVerificationService` — с rate-limit. - *реализовано, без rate-limit*
...
* [] Rate-limit / идемпотентность (поведение мидлварей). - *проверено базовое поведение*
...
1. Завершение API и Обработка Ошибок
* [x] Формат ошибок: Внедрить единый формат ошибок ProblemDetails (RFC 7807) для всего API.
* Идемпотентность: Реализовать middleware для поддержки идемпотентных запросов (через заголовок Idempotency-Key).

2. Безопасность
* Пароли: Убедиться, что используется Argon2id, и вынести его параметры в .env.
* Атрибут `#[SensitiveParameter]`: Пометить поля с паролями, чтобы они не попадали в логи.
* CORS и HSTS: Настроить более строгие политики безопасности для middleware.
* CAPTCHA: Добавить (опционально) защиту от ботов.

3. Тестирование
* Тесты на события: Написать тесты для обработчика события UserRegistered (который отправляет письмо).
* API тесты: Написать больше тестов для API, особенно негативных сценариев (неверные данные, повторные верификации и т.д.).
* Интеграционные тесты: Написать тесты на согласованность данных (например, user + token).

4. Документация и Качество Кода
* OpenAPI: Описать все эндпоинты в спецификации OpenAPI.
* README: Обновить документацию по проекту.
* Статический анализ: Прогнать phpstan и php-cs-fixer и исправить все найденные проблемы.
* Value Objects: Реализовать Email и PasswordHash как полноценные VO (вы ранее просили это пропустить, но в плане задача осталась).

5. Логи/Метрики
* Настроить логирование, сбор метрик (счетчики регистраций, ошибок) и трассировку запросов.
